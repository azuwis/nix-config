...
start() {
  ...
  test -z "$dns" && exit
  if ! grep -qFx "nameserver $dns" "$scidns_resolv" 2>/dev/null; then
...
stop() {
  ...
  rm -f "$scidns_resolv"
  networksetup -listallnetworkservices | grep -Fv '*' | while read -r service; do
    if [ "$(networksetup -getdnsservers "$service")" = "$scidns" ]; then

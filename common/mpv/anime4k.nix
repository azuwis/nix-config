...
let
  ...
  fast = {
    # curl -sL https://github.com/bloc97/Anime4K/raw/master/md/Template/GLSL_Mac_Linux_Low-end/input.conf | grep '^CTRL' | sed -r -e '/^$/d' -e 's|~~/shaders/|${anime4k}/|g' -e "s| |\" = ''|" -e 's|^|    "|' -e "s|$|'';|"
...
{
  options.my.mpv = {
    anime4k = mkEnableOption "anime4k" // {
      default = true;
    };
  };
